<script lang="ts">
  ...
  async function handleDelete() {
    ...
    try {
      ...
      alert(error instanceof Error ? error.message : $_('lorebook.deleteFailed'))
    } finally {
      deleting = false
...
<div class="flex h-full flex-col">
    ...
    <div class="flex min-w-0 items-center gap-3">
      ...
      <div class="min-w-0">
        <h2 class="text-surface-100 truncate font-semibold">
          {ui.lorebookEditMode ? $_('lorebook.editEntry') : entry.name}
        </h2>
        {#if !ui.lorebookEditMode}
        ...
        {:else}
          <button
            ...
            title={isLoreManagementActive
              ? $_('lorebook.editingDisabled')
              : $_('lorebook.editEntry')}
          >
          ...
          <button
            ...
            disabled={isLoreManagementActive}
            title={isLoreManagementActive ? $_('lorebook.editingDisabled') : $_('common.delete')}
          >
  ...
  <div class="flex-1 overflow-y-auto p-4">
    ...
    {:else}
      <div class="space-y-6">
        <div>
          <h3 class="text-surface-400 mb-2 text-sm font-medium">{$_('lorebook.description')}</h3>
          {#if entry.description}
            <p class="text-surface-200 whitespace-pre-wrap">{entry.description}</p>
          {:else}
            <p class="text-surface-500 italic">{$_('lorebook.noDescription')}</p>
          {/if}
        </div>

        {#if entry.aliases.length > 0}
          <div>
            <h3 class="text-surface-400 mb-2 text-sm font-medium">{$_('lorebook.aliases')}</h3>
        ...
        {#if entry.injection.keywords.length > 0}
          <div>
            <h3 class="text-surface-400 mb-2 text-sm font-medium">{$_('lorebook.keywords')}</h3>
            <div class="flex flex-wrap gap-2">
        ...
        <div>
          <h3 class="text-surface-400 mb-2 text-sm font-medium">
            {$_('lorebook.contextInclusion')}
          </h3>
          <div class="flex items-center gap-4 text-sm">
            <div>
              <span class="text-surface-500">{$_('lorebook.mode')}:</span>
              <span class="text-surface-300 ml-1">{injectionLabels[entry.injection.mode]}</span>
            </div>
            <div>
              <span class="text-surface-500">{$_('lorebook.priority')}:</span>
        ...
        {#if entry.loreManagementBlacklisted}
          ...
          >
            <span class="text-sm text-amber-400">{$_('lorebook.hiddenFromAILoreManagement')}</span>
          </div>
        {/if}

        {#if entry.hiddenInfo}
          <div>
            <h3 class="text-surface-400 mb-2 text-sm font-medium">{$_('lorebook.hiddenInfo')}</h3>
        ...
        <div>
          <h3 class="text-surface-400 mb-2 text-sm font-medium">{$_('lorebook.metadata')}</h3>
          <div class="text-surface-500 space-y-1 text-sm">
            <div>{$_('lorebook.created')}: {new Date(entry.createdAt).toLocaleDateString()}</div>
            <div>{$_('lorebook.updated')}: {new Date(entry.updatedAt).toLocaleDateString()}</div>
            <div>{$_('lorebook.source')}: <span class="capitalize">{entry.createdBy}</span></div>
            {#if entry.mentionCount > 0}
              <div>{$_('lorebook.mentions')}: {entry.mentionCount}</div>
            {/if}
          </div>
        </div>
      </div>
