<script lang="ts">
  ...
  async function deleteStory(storyId: string, event: MouseEvent) {
    ...
    const confirmed = await ask($_('library.delete.confirm'), {
      title: $_('library.delete.title'),
      kind: 'warning',
    })
    if (confirmed) {
...
<div class="bg-background relative h-full overflow-y-auto p-4 sm:p-6">
  <div class="mx-auto flex min-h-full max-w-5xl flex-col">
    ...
    <div class="mb-6 flex flex-row items-start justify-between gap-3 sm:mb-8 sm:gap-4">
      ...
      <div class="flex shrink-0 items-center gap-1.5 sm:gap-2">
        <Button
          ...
          variant="outline"
          title={$_('library.actions.syncStories')}
          onclick={() => ui.openSyncModal()}
        />
        <Button
          icon={Archive}
          label={$_('library.vault')}
          variant="outline"
          title={$_('library.actions.openVault')}
          onclick={() => ui.setActivePanel('vault')}
        />
        <Button
          icon={Upload}
          label={$_('library.import')}
          variant="outline"
          title={$_('library.actions.importStory')}
        ...
        <Button
          ...
          label={$_('library.stImport')}
          variant="outline"
          title={$_('library.actions.importFromSillyTavern')}
          onclick={openSTImportWizard}
        />
        <Button
          variant="default"
          icon={Plus}
          label={$_('library.newStory')}
          title={$_('library.actions.createNewStory')}
    ...
    {#if story.allStories.length === 0}
      <EmptyState
        icon={BookOpen}
        title={$_('library.emptyState.title')}
        description={$_('library.emptyState.description')}
        actionLabel={$_('library.emptyState.action')}
  ...
  >
    ...
    <span class="hidden sm:inline">{$_('library.discord')}</span>
  </a>
</div>
